...
const quickAccess=['📄 Nova PMC','📋 Meus pedidos','◔ Budget (90 dias)','ⓢ Adicionar orçamento','↩ Devolver para ajuste','☷ Status dos pedidos','▣ Gerar Word / PDF','⚒ O que é GISU?'];
const suggestions={dashboard:quickAccess,nova:quickAccess,rascunhos:quickAccess,solicitacoes:quickAccess,referencias:quickAccess,compradora:quickAccess,usuarios:quickAccess,config:quickAccess,detalhe:quickAccess,default:quickAccess};
...
const quickQuestionIntent={'nova pmc':'criar_pmc','meus pedidos':'acompanhar','budget 90 dias':'budget','adicionar orcamento':'orcamento','devolver para ajuste':'devolver','status dos pedidos':'status','gerar word pdf':'word','o que e gisu':'gisu'};
...
function addMessage(text,who='bot',save=true){const box=$('#pmcAssistantMessages');if(!box)return;const row=document.createElement('div');row.className=`pmc-chat-row ${who}`;row.innerHTML=`<div class="pmc-chat-bubble">${format(text)}<span class="pmc-chat-time">${currentTime()}${who==='user'?' ✓✓':''}</span></div>`;box.appendChild(row);box.scrollTop=box.scrollHeight;if(save)try{const h=JSON.parse(sessionStorage.getItem('pmc_ai_history')||'[]');h.push({text,who});sessionStorage.setItem('pmc_ai_history',JSON.stringify(h.slice(-20)));}catch(_){}}
function ask(q){q=String(q||'').trim();if(!q)return;addMessage(q,'user');setTimeout(()=>addMessage(answer(q),'bot'),160);}
function updateContext(){const p=activePage(),ctx=$('#pmcAssistantContext'),list=$('#pmcAssistantSuggestions');if(!ctx||!list)return;ctx.innerHTML=`Você está em: <strong>${pageNames[p]||'PMC Digital'}</strong>`;list.innerHTML=(suggestions[p]||suggestions.default).map(x=>`<button type="button">${escapeHtml(x)}</button>`).join('');list.querySelectorAll('button').forEach(b=>b.onclick=()=>ask(b.textContent.replace(/^[^A-Za-zÀ-ÿ0-9]+\s*/,'')));}
function init(){const launcher=$('#pmcAssistantLauncher'),panel=$('#pmcAssistantPanel'),close=$('#pmcAssistantClose'),form=$('#pmcAssistantForm'),input=$('#pmcAssistantInput'),clear=$('#pmcAssistantClear'),attach=document.querySelector('.pmc-assistant-attach');if(!launcher)return;const open=()=>{panel.hidden=false;launcher.hidden=true;launcher.setAttribute('aria-expanded','true');$('#pmcAssistantBadge').hidden=true;updateContext();setTimeout(()=>input.focus(),70)};const shut=()=>{panel.hidden=true;launcher.hidden=false;launcher.setAttribute('aria-expanded','false')};launcher.onclick=open;close.onclick=shut;form.onsubmit=e=>{e.preventDefault();const q=input.value.trim();if(!q)return;input.value='';input.style.height='auto';ask(q)};input.oninput=()=>{input.style.height='auto';input.style.height=Math.min(input.scrollHeight,90)+'px'};input.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();form.requestSubmit()}};if(attach)attach.onclick=()=>addMessage('Os arquivos devem ser anexados diretamente nos campos da PMC, orçamento, pedido de compra ou NFE.','bot');clear.onclick=()=>{sessionStorage.removeItem('pmc_ai_history');$('#pmcAssistantMessages').innerHTML='';addMessage(`Olá${firstName()?', '+firstName():''}! 👋\nSou seu assistente do PMC Digital.\nComo posso te ajudar hoje?`);updateContext()};let restored=false;try{const h=JSON.parse(sessionStorage.getItem('pmc_ai_history')||'[]');if(h.length){h.forEach(m=>addMessage(m.text,m.who,false));restored=true}}catch(_){}if(!restored)addMessage(`Olá${firstName()?', '+firstName():''}! 👋\nSou seu assistente do PMC Digital.\nComo posso te ajudar hoje?`);updateContext();new MutationObserver(updateContext).observe(document.querySelector('main')||document.body,{subtree:true,attributes:true,attributeFilter:['class']});}
