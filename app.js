...
function renderDashboard(){
  const mine=meusPedidos(); const all=state.solicitacoes; const f=filtrarDashboard(all); const mineFiltradas=filtrarDashboard(mine);
  const allItens=allItems(all); const fItens=allItems(f);
  ...
  $('#meusPedidosList').innerHTML=mineFiltradas.slice(0,12).map(pmcMiniCard).join('') || '<p>Nenhuma PMC sua encontrada.</p>';
  $('#recentList').innerHTML=f.slice(0,18).map(pmcMiniCard).join('') || '<p>Nenhuma PMC encontrada.</p>';
...
function pmcMiniCard(s){
  ...
  return `<article class="mini-item pmc-mini-card ${corStatus}"><div class="pmc-mini-main"><div><b class="pmc-number">PMC ${esc(s.numeroPedido||'Rascunho')}</b> ${s.temAlerta?'<span class="alert">⚠ 90 dias</span>':''}<br>${badge(s.status||'Pendente')}<br><small>${itens.length} produto${itens.length===1?'':'s'} • Família: ${esc(familias.join(', ')||'Não informada')}</small><br><small>Solicitante: ${esc(s.solicitante||'-')} • Pedido: ${fmtDate(s.criadoEm)}${compradores.length?' • Compradora: '+esc(compradores.join(', ')):''}</small></div><button class="primary" type="button" onclick="openDetail('${s.id}')">Abrir PMC</button></div></article>`;
}
...
function renderSolicitacoes(){
  ...
  let rows=state.solicitacoes.filter(s=>(!st||s.status===st||s.itens?.some(i=>(i.status||s.status)===st))&&(!fam||s.itens?.some(i=>familiaCodigo(i.familia)===fam))&&(!q||norm(searchText(s)).includes(q)));
...
window.reenviarPmcEmail=async function(id){
  if(state.user?.perfil!=='admin') return toast('Apenas administradores podem reenviar a PMC.');
  const s=state.solicitacoes.find(x=>x.id===id); if(!s||s.status==='Rascunho') return toast('PMC não encontrada ou ainda está em rascunho.');
  confirmAction(`Reenviar a PMC ${s.numeroPedido||''} para ${state.config.emailCompradora||'o e-mail configurado da compradora'}?`,async()=>{
    const btns=[...document.querySelectorAll('.admin-resend-btn')]; btns.forEach(b=>b.disabled=true);
    try{const resultado=await enviarNotificacaoCompradora(s); renderAll(); openDetail(s.id); toast(resultado===true?'PMC reenviada por e-mail com sucesso.':resultado===false?'Não foi possível reenviar o e-mail. Confira o EmailJS.':'Configure o EmailJS e o e-mail da compradora antes de reenviar.');}
...
window.savePmcGisu=async function(id){
  const s=state.solicitacoes.find(x=>x.id===id); if(!s) return toast('PMC não encontrada.');
  const marcado=!!$('#pmcGisu')?.checked; const valor=Number($('#pmcGisuValor')?.value||0);
  if(marcado && valor<=0) return toast('Informe o valor total da compra realizada via GISU.');
  s.gisu=marcado; s.gisuValor=marcado?valor:0; s.gisuData=marcado?(s.gisuData||new Date().toISOString().slice(0,10)):''; s.gisuCompradora=marcado?(state.user?.nome||''):'';
  s.historico=s.historico||[]; s.historico.push(log(marcado?`PMC marcada como GISU. Valor realizado: ${money(valor)}`:'Marcação GISU removida da PMC.'));
  await persistSolicitacao(s); renderAll(); openDetail(id); toast(marcado?'GISU registrado com sucesso. Este valor não reduz o budget de R$ 3.000,00.':'GISU removido da PMC.');
};
...
function quoteAnalysisHtml(i){
  const q=quoteStats(i); if(!q.count) return '<div class="quote-empty">Adicione manualmente os orçamentos recebidos para comparar este produto.</div>';
...
function manualQuoteRow(itemId,index,quantidade){
  return `<div class="manual-quote-row" data-quote-index="${index}"><div class="manual-quote-heading"><b>Fornecedor ${index+1}</b><button class="manual-quote-remove" type="button" onclick="removeManualQuote(this)">Remover</button></div><div class="editor-grid quote-form"><label>Fornecedor<input class="manual-fornecedor" placeholder="Nome do fornecedor"></label><label>Quantidade cotada<input class="manual-qtd" type="number" step="0.001" min="0" value="${Number(quantidade||1)}"></label><label>Valor unitário (R$)<input class="manual-unitario" type="number" step="0.01" min="0" placeholder="0,00"></label><label>Valor total deste produto (R$)<input class="manual-total" type="number" step="0.01" min="0" placeholder="0,00"></label><label class="wide">Link do orçamento (opcional)<input class="manual-url" type="url" placeholder="https://..."></label></div></div>`;
}
window.addManualQuote=function(itemId){
  const list=document.getElementById(`manualQuotes_${itemId}`); if(!list) return;
  const item=state.solicitacoes.flatMap(s=>s.itens||[]).find(i=>i.id===itemId);
  const index=list.querySelectorAll('.manual-quote-row').length;
  list.insertAdjacentHTML('beforeend',manualQuoteRow(itemId,index,item?.quantidade||1));
  renumberManualQuotes(list);
};
window.removeManualQuote=function(button){
  const row=button.closest('.manual-quote-row'); const list=row?.parentElement; if(!row||!list) return;
  const rows=list.querySelectorAll('.manual-quote-row');
  if(rows.length===1){ row.querySelectorAll('input').forEach(input=>{input.value=input.classList.contains('manual-qtd')?input.defaultValue:'';}); return; }
  row.remove(); renumberManualQuotes(list);
};
function renumberManualQuotes(list){
  [...list.querySelectorAll('.manual-quote-row')].forEach((row,index)=>{row.dataset.quoteIndex=index; const title=row.querySelector('.manual-quote-heading b'); if(title) title.textContent=`Fornecedor ${index+1}`;});
}
function itemEditor(sid,i,idx){
  ...
  <div class="supplier-doc-box"><div class="supplier-title"><div><h5>1. Orçamentos deste produto</h5><p>Cadastre manualmente os valores recebidos de cada fornecedor. O sistema compara o preço unitário deste item.</p></div><button class="template-download" type="button" onclick="downloadPedidoWord('${sid}')">Gerar modelo padrão de cotação</button></div>${quoteAnalysisHtml(i)}
  <div id="manualQuotes_${i.id}" class="manual-quotes-list">${manualQuoteRow(i.id,0,i.quantidade)}</div>
  <div class="manual-quote-actions"><button type="button" onclick="addManualQuote('${i.id}')">+ Adicionar mais um fornecedor</button><span>Você pode cadastrar vários orçamentos antes de salvar.</span></div>${documentosHtml(i)}</div>
...
window.saveItemStatus=async function(sid,itemId){
  ...
  const quoteRows=[...document.querySelectorAll(`#manualQuotes_${itemId} .manual-quote-row`)];
  const novosOrcamentos=[];
  for(const row of quoteRows){
    const fornecedor=row.querySelector('.manual-fornecedor')?.value.trim()||'';
    const qtdCotada=Number(row.querySelector('.manual-qtd')?.value||i.quantidade||1);
    let valorUnitario=Number(row.querySelector('.manual-unitario')?.value||0);
    let valorOrcado=Number(row.querySelector('.manual-total')?.value||0);
    const docUrl=row.querySelector('.manual-url')?.value.trim()||'';
    const preenchido=fornecedor||valorUnitario||valorOrcado||docUrl;
    if(!preenchido) continue;
    if(!fornecedor) return toast('Informe o nome de todos os fornecedores cadastrados.');
    if(!valorUnitario&&!valorOrcado) return toast(`Informe o valor do orçamento de ${fornecedor}.`);
    if(!valorUnitario&&valorOrcado&&qtdCotada) valorUnitario=valorOrcado/qtdCotada;
    if(!valorOrcado&&valorUnitario&&qtdCotada) valorOrcado=valorUnitario*qtdCotada;
    novosOrcamentos.push({id:crypto.randomUUID(),fornecedor,quantidadeCotada:qtdCotada,valorUnitario,valorTotal:valorOrcado,nomeArquivo:docUrl?'Documento online':'Orçamento informado manualmente',tipo:docUrl?'link':'manual',urlDocumento:docUrl,enviadoEm:new Date().toISOString(),revisadoPor:state.user.nome});
  }
  ...
  if(novosOrcamentos.length) i.documentosFornecedores.push(...novosOrcamentos);
...
function resumoBudgetFamilia(familia){
  ...
  const gisuCompras=state.solicitacoes.filter(s=>s.gisu&&Number(s.gisuValor||0)>0&&familiaCodigo((s.itens||[])[0]?.familia)===familiaCodigo(familia)).sort((a,b)=>parseDateLocal(a.gisuData||a.criadoEm)-parseDateLocal(b.gisuData||b.criadoEm));
  const gisuTotal=gisuCompras.reduce((t,s)=>t+Number(s.gisuValor||0),0);
  ...
  return {familia, compras, limite, usado, disponivel, ultima, proxima, percentual, situacao, gisuCompras, gisuTotal};
}
function renderReferencias(){
  ...
  $('#budgetTable tbody').innerHTML=rows.map(x=>{
    ...
    return `<tr><td><b>${esc(x.codigo)}</b></td><td>${esc(x.descricao)}</td><td>${money(x.limite)}</td><td><b>${money(x.usado)}</b><div class="budget-progress"><span style="width:${Math.min(100,x.percentual*100)}%"></span></div></td><td class="budget-available">${money(x.disponivel)}</td><td class="budget-gisu"><b>${money(x.gisuTotal)}</b>${x.gisuCompras.length?`<small>${x.gisuCompras.length} PMC(s) via licitação</small>`:'<small>Nenhuma compra GISU</small>'}</td><td>${x.ultima?fmtDate(x.ultima):'-'}</td><td>${x.proxima?fmtDate(x.proxima):'-'}</td><td><span class="family-balance ${cls}">${label}</span></td><td><button onclick="openBudgetDetail('${escAttr(x.codigo)}')">Ver histórico</button></td></tr>`;
  }).join('')||'<tr><td colspan="10">Nenhuma família encontrada.</td></tr>';
}
window.openBudgetDetail=function(codigo){
  ...
  $('#detailContent').innerHTML=`<div class="budget-detail-head"><p class="eyebrow">Budget da família</p><h2>${esc(fam?fam.codigo+' - '+fam.descricao:codigo)}</h2></div><div class="cards budget-detail-cards"><div class="card kpi"><span>Limite</span><b>${money(x.limite)}</b></div><div class="card kpi"><span>Utilizado</span><b>${money(x.usado)}</b></div><div class="card kpi"><span>Disponível</span><b>${money(x.disponivel)}</b></div><div class="card kpi gisu-kpi"><span>Compras GISU</span><b>${money(x.gisuTotal)}</b></div></div><p><b>Próxima liberação estimada:</b> ${x.proxima?fmtDate(x.proxima):'Não há valor comprometido no período.'}</p><div class="table-wrap"><table><thead><tr><th>Finalização</th><th>Produto</th><th>Solicitante</th><th>Compradora</th><th>Valor</th></tr></thead><tbody>${linhas}</tbody></table></div>${x.gisuCompras.length?`<h3 class="gisu-history-title">Compras realizadas via GISU / Licitação</h3><div class="table-wrap"><table><thead><tr><th>PMC</th><th>Data do registro</th><th>Solicitante</th><th>Compradora</th><th>Valor GISU</th></tr></thead><tbody>${x.gisuCompras.map(s=>`<tr><td><b>PMC ${esc(s.numeroPedido||'-')}</b></td><td>${s.gisuData?fmtDate(s.gisuData):fmtDate(s.criadoEm)}</td><td>${esc(s.solicitante||'-')}</td><td>${esc(s.gisuCompradora||s.comprador||'-')}</td><td><b>${money(s.gisuValor||0)}</b></td></tr>`).join('')}</tbody></table></div>`:''}`;
